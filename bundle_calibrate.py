...
class CommandLineParams:
    def __init__(self) -> None:
        ...
        ap.add_argument("-d","--draw",required=False,help="path to save detect result",)
        ...
        for file,image in bundle:
            ...
            for tag_results, tag_type, obj_pts in zip(*result):
                for tag in tag_results:
                    ...
                    # undistort 
                    corners=cv2.undistortImagePoints(corners,camera.cameraMatrix,camera.distCoeffs).reshape((4,2))
                    # print(corners)
                    if cmd_params.draw:
                        for p in corners:
                            cv2.circle(img_show,(int(p[0]),int(p[1])),cmd_params.draw_width,(0,50,255),int(cmd_params.draw_width/3)+1)
                    rvec, tvec = solve_pnp(
